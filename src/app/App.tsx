...
export default function App() {
  ...
  return <div className="app"><Sidebar /><main><Topbar onNew={() => setCreating(true)} /><div className="content">{query && view !== 'about' && view !== 'concept-map' && <div className="search-result"><Search size={16} />{filtered.length} resultados para "{query}"</div>}{view === 'timeline' && <Timeline />}{view === 'calendar' && <CalendarView />}{view === 'board' && <Board />}{view === 'concept-map' && <ConceptMap />}{view === 'library' && <LibraryView />}{view === 'about' && <AboutUpdates />}</div></main>{(creating || selected) && <Editor initial={selected} onClose={() => { setCreating(false); select(null) }} />}</div>
}
