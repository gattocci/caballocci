...
export function Editor({ initial, onClose }: { initial: Post | null; onClose(): void }) {
  ...
  const catalogPreview = { source: 'caballocci', dry_run: true, publish: false, items: [{ kind: catalogKind, external_id: `post-${initial?.id || ''}`, title: draft.title, ...(catalogKind === 'resource' && catalogCategoryId ? { category_id: Number(catalogCategoryId) } : {}), summary: catalogSummary, content: catalogContent, ...(catalogKind === 'resource_lite' ? { copy_text: catalogContent, copy_label: 'Copiar' } : {}) }] }
