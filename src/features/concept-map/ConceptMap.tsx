...
const defaultCamera = { x: -120, y: -80, zoom: 1 }
...
type Camera = typeof defaultCamera

function nextPosition(count: number) { return { x: 52 + (count % 5) * 270, y: 92 + Math.floor(count / 5) * 182 } }
function clampZoom(value: number) { return Math.min(1.8, Math.max(.45, Math.round(value * 100) / 100)) }
function cameraKey(folderId: string | null) { return `caballocci.map-camera.${folderId || 'root'}` }
function loadCamera(folderId: string | null): Camera {
  try {
    const saved = JSON.parse(localStorage.getItem(cameraKey(folderId)) || '') as Camera
    if (Number.isFinite(saved.x) && Number.isFinite(saved.y) && Number.isFinite(saved.zoom)) return { x: saved.x, y: saved.y, zoom: clampZoom(saved.zoom) }
  } catch { /* Use the initial camera. */ }
...
export function ConceptMap() {
  ...
  const [camera, setCamera] = useState<Camera>(loadCamera(null))
  const [viewport, setViewport] = useState({ width: 1000, height: 610 })
  ...
  const [pan, setPan] = useState<{ startX: number; startY: number; camera: Camera } | null>(null)
  ...
  const folderNodes = useMemo(() => nodes.filter(node => node.folderId === activeFolderId), [activeFolderId, nodes])
  ...
  const visibleWorld = useMemo(() => {
    const margin = 300 / camera.zoom; const right = camera.x + viewport.width / camera.zoom; const bottom = camera.y + viewport.height / camera.zoom
    return folderNodes.filter(node => node.x + nodeSize.width >= camera.x - margin && node.x <= right + margin && node.y + nodeSize.height >= camera.y - margin && node.y <= bottom + margin)
  }, [camera, folderNodes, viewport])
  const selected = folderNodes.find(node => node.id === selectedId) || null

  const persistCamera = (next: Camera) => localStorage.setItem(cameraKey(activeFolderId), JSON.stringify(next))
  const setCameraAndPersist = (next: Camera) => { cameraRef.current = next; setCamera(next); persistCamera(next) }
  ...
  useEffect(() => {
    setSelectedId(null); setLinkStartId(null)
    const next = loadCamera(activeFolderId); cameraRef.current = next; setCamera(next)
  }, [activeFolderId])
  useEffect(() => {
  ...
  const addNode = async (kind: ConceptNodeKind, sourceId: string | null, title: string, body: string) => {
    const saved = await window.planner.conceptMap.saveNode({ folderId: activeFolderId, kind, sourceId, title: title.trim(), body: body.trim(), ...nextPosition(folderNodes.length) })
  ...
  const createIdea = async () => { if (!ideaTitle.trim()) return; const idea = await saveIdea({ space: activeSpace || 'Mi contenido', title: ideaTitle.trim(), body: ideaBody.trim(), tags: [], media: [], status: 'inbox', priority: 'normal', dueDate: null }); await addNode('idea', idea.id, idea.title, idea.body); setIdeaTitle(''); setIdeaBody(''); setIdeaOpen(false) }
  const createFolder = async () => { if (!folderName.trim()) return; const folder = await window.planner.conceptMap.saveFolder({ parentId: activeFolderId, name: folderName.trim() }); setFolders(current => [...current, folder]); setFolderName(''); setFolderOpen(false) }
  const deleteActiveFolder = async () => { if (!activeFolderId || !window.confirm('Eliminar esta carpeta? Sus nodos y subcarpetas subiran al nivel anterior.')) return; const result = await window.planner.conceptMap.removeFolder(activeFolderId); await reload(); setActiveFolderId(result.parentId) }
  const selectNode = async (id: string) => {
    if (linkStartId && linkStartId !== id) { const exists = links.some(link => link.fromNodeId === linkStartId && link.toNodeId === id && link.relation === relation); if (!exists) { const saved = await window.planner.conceptMap.saveLink({ fromNodeId: linkStartId, toNodeId: id, relation }); setLinks(current => [...current, saved]) }; setLinkStartId(null); return }
    setSelectedId(id)
  }
  const handlePointerMove = (event: PointerEvent<HTMLDivElement>) => {
    if (drag) { const x = drag.nodeX + (event.clientX - drag.startX) / cameraRef.current.zoom; const y = drag.nodeY + (event.clientY - drag.startY) / cameraRef.current.zoom; setNodes(current => current.map(node => node.id === drag.id ? { ...node, x, y } : node)); return }
    if (pan) { const next = { ...pan.camera, x: pan.camera.x - (event.clientX - pan.startX) / pan.camera.zoom, y: pan.camera.y - (event.clientY - pan.startY) / pan.camera.zoom }; cameraRef.current = next; setCamera(next) }
  }
  const finishPointer = async () => {
    if (pan) persistCamera(cameraRef.current); setPan(null)
    if (!drag) return
    const node = nodes.find(item => item.id === drag.id); setDrag(null); if (!node) return
  ...
  const beginPan = (event: PointerEvent<HTMLDivElement>) => { if ((event.target as HTMLElement).closest('.concept-node, .concept-link')) return; event.currentTarget.setPointerCapture(event.pointerId); setPan({ startX: event.clientX, startY: event.clientY, camera: cameraRef.current }) }
  const zoomAt = (nextZoom: number, clientX = viewport.width / 2, clientY = viewport.height / 2) => {
    const element = viewportRef.current; const rect = element?.getBoundingClientRect(); const localX = rect ? clientX - rect.left : viewport.width / 2; const localY = rect ? clientY - rect.top : viewport.height / 2; const previous = cameraRef.current; const zoom = clampZoom(nextZoom); const worldX = previous.x + localX / previous.zoom; const worldY = previous.y + localY / previous.zoom; setCameraAndPersist({ x: worldX - localX / zoom, y: worldY - localY / zoom, zoom })
  }
  const onWheel = (event: WheelEvent<HTMLDivElement>) => { event.preventDefault(); zoomAt(cameraRef.current.zoom * (event.deltaY < 0 ? 1.12 : .89), event.clientX, event.clientY) }
  const fitContent = () => {
    if (!folderNodes.length) { setCameraAndPersist({ ...defaultCamera }); return }
    const left = Math.min(...folderNodes.map(node => node.x)); const top = Math.min(...folderNodes.map(node => node.y)); const right = Math.max(...folderNodes.map(node => node.x + nodeSize.width)); const bottom = Math.max(...folderNodes.map(node => node.y + nodeSize.height)); const zoom = clampZoom(Math.min(viewport.width / (right - left + 160), viewport.height / (bottom - top + 160))); setCameraAndPersist({ x: (left + right) / 2 - viewport.width / zoom / 2, y: (top + bottom) / 2 - viewport.height / zoom / 2, zoom })
  }
  ...
  return <section className="workspace concept-workspace"><ViewHeading title="Mapa conceptual" subtitle="Conecta ideas, publicaciones y recursos para ver su contexto y dependencias."><button className="new-button" onClick={() => setIdeaOpen(true)}><Lightbulb size={17} /> Nueva idea</button><button className="new-button" onClick={() => setResourceOpen(true)}><Plus size={17} /> Recurso general</button><button className="map-action" onClick={() => setFolderOpen(true)}><FolderPlus size={16} /> Nueva carpeta</button></ViewHeading>
    ...
    <div className="concept-toolbar"><div className="relation-control" role="group" aria-label="Tipo de relacion">{(Object.keys(relationLabels) as ConceptRelation[]).map(value => <button key={value} className={relation === value ? 'active' : ''} onClick={() => setRelation(value)}>{relationLabels[value]}</button>)}</div><button className={'map-action ' + (linkStartId ? 'active' : '')} disabled={!selected} onClick={() => setLinkStartId(current => current ? null : selected?.id || null)}><Link2 size={16} />{linkStartId ? 'Elegir destino' : 'Conectar seleccionado'}</button>{selected && <button className="map-action danger" onClick={() => void removeSelected()}><Trash2 size={16} /> Quitar del mapa</button>}<div className="map-zoom"><button title="Alejar" onClick={() => zoomAt(cameraRef.current.zoom / 1.2)}><Minus size={16} /></button><span>{Math.round(camera.zoom * 100)}%</span><button title="Acercar" onClick={() => zoomAt(cameraRef.current.zoom * 1.2)}><ZoomIn size={16} /></button><button title="Encuadrar contenido" onClick={fitContent}><Maximize2 size={15} /></button></div></div>
    <div className="concept-map-viewport" ref={viewportRef} style={{ backgroundSize: `${20 * camera.zoom}px ${20 * camera.zoom}px`, backgroundPosition: `${-camera.x * camera.zoom}px ${-camera.y * camera.zoom}px` }} onPointerDown={beginPan} onPointerMove={handlePointerMove} onPointerUp={() => void finishPointer()} onPointerCancel={() => void finishPointer()} onWheel={onWheel}><svg className="concept-links" aria-hidden="true" width={viewport.width} height={viewport.height}><defs><marker id="concept-arrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z" /></marker></defs>{links.map(link => { const from = visibleWorld.find(node => node.id === link.fromNodeId); const to = visibleWorld.find(node => node.id === link.toNodeId); if (!from || !to) return null; const x1 = (from.x + nodeSize.width - camera.x) * camera.zoom; const y1 = (from.y + 68 - camera.y) * camera.zoom; const x2 = (to.x + 6 - camera.x) * camera.zoom; const y2 = (to.y + 68 - camera.y) * camera.zoom; const middleX = (x1 + x2) / 2; const middleY = (y1 + y2) / 2; return <g className="concept-link" key={link.id}><path d={`M ${x1} ${y1} C ${middleX} ${y1}, ${middleX} ${y2}, ${x2} ${y2}`} markerEnd="url(#concept-arrow)" /><foreignObject x={middleX - 55} y={middleY - 12} width="110" height="25"><button title="Quitar relacion" onClick={() => void removeLink(link.id)}>{relationLabels[link.relation]}</button></foreignObject></g> })}</svg>{visibleWorld.map(node => { const Icon = kindIcons[node.kind]; return <article key={node.id} className={'concept-node ' + node.kind + (selectedId === node.id ? ' selected' : '') + (linkStartId === node.id ? ' link-source' : '')} style={{ transform: `translate3d(${(node.x - camera.x) * camera.zoom}px, ${(node.y - camera.y) * camera.zoom}px, 0) scale(${camera.zoom})` }} onClick={() => void selectNode(node.id)} onPointerDown={event => { if ((event.target as HTMLElement).closest('button')) return; event.stopPropagation(); event.currentTarget.setPointerCapture(event.pointerId); setDrag({ id: node.id, startX: event.clientX, startY: event.clientY, nodeX: node.x, nodeY: node.y }) }}><header><span><Icon size={14} /> {kindLabels[node.kind]}</span><GitBranch size={15} /></header><strong>{node.title}</strong>{node.body && <p>{node.body}</p>}</article> })}{!folderNodes.length && <div className="concept-empty"><Network size={30} /><strong>Esta carpeta esta vacia</strong><span>Anade una idea, publicacion o recurso general.</span></div>}</div>
