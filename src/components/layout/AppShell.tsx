...
import { CalendarDays, Clock3, Columns3, Database, FolderOpen, Library, Lightbulb, Menu, MoreHorizontal, Network, Pencil, Plus, Search, Settings, Trash2, X } from 'lucide-react'
import { usePlanner } from '../../app/store'
import { getPlatformMeta } from '../../shared/constants'
import type { Platform } from '../../shared/types'
import './app-shell.css'

export function PlatformMark({ platform, small = false }: { platform: Platform; small?: boolean }) {
  const meta = getPlatformMeta(platform)
  return <span className={'platform-mark ' + platform + (small ? ' small' : '')}>{meta.mark}</span>
}

export function Sidebar() {
  const { view, setView, posts, ideas, activeSpace, customSpaces, hiddenSpaces, spaceNotes, spaceKey, saveSpaceNotes, setActiveSpace, addSpace, renameSpace, removeSpace } = usePlanner()
  ...
  return <aside className="sidebar">
    ...
    <div className="side-section"><p>ESPACIOS</p><button className={'space ' + (activeSpace === null ? 'active' : '')} onClick={() => setActiveSpace(null)}><span className="space-dot coral">C</span><span>Todos los contenidos</span></button>{spaces.map((space, index) => <div className="space-row" key={space}><button className={'space ' + (activeSpace === space ? 'active' : '')} onClick={() => setActiveSpace(space)}><span className={'space-dot ' + (index % 2 ? 'coral' : 'mint')}>{space.slice(0, 1).toUpperCase()}</span><span>{space}</span></button><button className="space-menu-button" aria-label={'Opciones de ' + space} aria-expanded={menuOpen === space} onClick={() => setMenuOpen(menuOpen === space ? null : space)}><MoreHorizontal size={16} /></button>{menuOpen === space && <div className="space-menu"><button onClick={() => { setEditingSpace(space); setSpaceName(space); setMenuOpen(null) }}><Pencil size={14} /> Renombrar</button><button className="danger-menu-item" onClick={() => { setRemovingSpace(space); setMenuOpen(null) }}><Trash2 size={14} /> Quitar espacio</button></div>}</div>)}<button className="add-space" onClick={() => { setCreatingSpace(true); setSpaceName('') }}><Plus size={15} /> Nuevo espacio</button>{activeSpace && <div className="space-context"><div className="space-context-heading"><strong>{activeSpace}</strong><button className="icon-button" title="Abrir carpeta del espacio" onClick={() => void window.planner.system.openSpaceFolder(activeSpace)}><FolderOpen size={15} /></button></div><div className="space-context-stats"><span><b>{posts.filter(post => post.project === activeSpace).length}</b> publicaciones</span><span><b>{ideas.filter(idea => idea.space === activeSpace).length}</b> ideas</span></div><label><span>NOTAS DEL ESPACIO</span><textarea value={spaceNotes[spaceKey(activeSpace)] || ''} onChange={event => saveSpaceNotes(activeSpace, event.target.value)} placeholder="Objetivo, tono, enlaces, decisiones y contexto..." /></label><small className="space-folder-hint">La carpeta del espacio contiene las carpetas de sus publicaciones.</small></div>}</div>
...
export function Topbar({ onNew, onSearchSubmit }: { onNew: () => void; onSearchSubmit?: () => void }) {
  ...
  return <header className="topbar"><div className="search"><Search size={17} /><input value={query} onChange={e => setQuery(e.target.value)} onKeyDown={event => { if (event.key === 'Enter') onSearchSubmit?.() }} placeholder="Buscar publicaciones, ideas, etiquetas..." /><kbd>Ctrl K</kbd></div><div className="top-actions"><div className="app-menu-wrap"><button className="icon-button" title="Abrir menú" aria-expanded={open} onClick={() => setOpen(value => !value)}><Menu size={18} /></button>{open && <div className="app-menu"><button onClick={() => go('timeline')}><Clock3 size={15} /> Línea de tiempo</button><button onClick={() => go('calendar')}><CalendarDays size={15} /> Calendario</button><button onClick={() => go('board')}><Columns3 size={15} /> Tablero</button><button onClick={() => go('ideas')}><Lightbulb size={15} /> Ideas</button><button onClick={() => go('library')}><Library size={15} /> Biblioteca</button><hr /><button onClick={() => go('about')}><Settings size={15} /> Acerca de y actualizaciones</button></div>}</div><button className="new-button" onClick={onNew}><Plus size={17} /> Nueva publicación</button><div className="avatar">CG</div></div></header>
}
